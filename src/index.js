...
app.get('/Account', async (req, res) => {
   ...
   if(req.session.username !== undefined)
   {
      ...
      else //requested someone else's account
      {
         ...
         {
            var res1 = await client.query('SELECT * FROM users WHERE username= $1', [acc]); //DATABASE CONNECTION ARE IN DIFFERENT THREADS, NEED MANUAL TERAPY
            if(res1.rows.length>0)
            {
               ...
               var followed = false;
               var res2 = await client.query('SELECT * FROM follows WHERE username= $1 AND follows = $2', [req.session.username,acc]); //DATABASE CONNECTION ARE IN DIFFERENT THREADS, NEED MANUAL TERAPY
               ...
               for(var i=0;i<res2.rows.length;i++)
               {
                  ...
                  if(res2.rows[i]["postedby"]!==req.session.username)
                  {
                     ...
                     var save = "Save";
                     var share = "Share";
                     var res3 = await client.query('SELECT * FROM likes WHERE username = $1 AND pid= $2', [req.session.username,res2.rows[i]["pid"]]); //DATABASE CONNECTION ARE IN DIFFERENT THREADS, NEED MANUAL TERAPY
...
app.post('/update_user_data', async (req,res) => {
   if(req.session.username !== undefined)
   {
      ...
      if(webData.info !== undefined)
      {
         ...
         {
            var fname = webData.info;
            var tbull = true;
            if(!clean_fullname(fname) || fname.length > 100 || fname.length < 3)
            {
               tbull = false;
            }
            if(tbull)
         ...
         {
            client.query('UPDATE users set banned = 1 WHERE username = $1', [req.session.username]);
            req.session.username = undefined;
            const obj = `{"response": "0"}`;
            res.send(obj);
         ...
         else if(webData.infoType !== undefined && webData.infoType === "follow")
         {
            ...
            var followed = false;
            var res2 = await client.query('SELECT * FROM follows WHERE username= $1 AND follows = $2', [req.session.username,webData.info]); //DATABASE CONNECTION ARE IN DIFFERENT THREADS, NEED MANUAL TERAPY
            if (res2.rows.length > 0) {followed = true;}
            if (followed)
            {
               client.query('DELETE FROM follows WHERE username = $1 AND follows = $2', [req.session.username,webData.info]);
...
app.post('/update_post_data', async (req,res) => {
   if(req.session.username !== undefined)
   {
      ...
      if(webData.action !== undefined && webData.action === 'addpost')
      {
         ...
         {
            client.query('INSERT INTO posts (title, content, postedby, publishdate) values($1, $2, $3, NOW())', [webData.title, webData.content, req.session.username]);
            const obj = `{"response": "0"}`;
            res.send(obj);
         }
      }
...
app.post('/admin_update', async (req,res) => {
   if(req.session.username !== undefined && req.session.admin !== undefined)
   {
      ...
      if(webData.action !== undefined && webData.action === 'updatepost')
      {
         if(webData.content !== undefined && webData.content.length > 9 && webData.content.length <= 5000 && webData.title !== undefined && webData.title.length > 4 && webData.title.length < 200 && webData.post1 !== undefined && clean(webData.post1))
         {
            client.query('UPDATE posts SET title=$1, content=$2 WHERE pid = $3', [webData.title, webData.content, webData.post1]);
            const obj = `{"response": "0"}`;
            res.send(obj);
         }
      }
      ...
      else if(webData.action !== undefined && webData.action === 'updateuser')
      {
         ...
         var pass = webData.pass;
         var fname = webData.fname;
         var mail = webData.mail;
         if(pass.length >= 6 && clean(pass) && mail.length >= 6 && clean(mail) && mail.indexOf('@') >= 0 && mail.indexOf('.') >= 0 && fname.length >= 3 && clean_fullname(fname) && clean(user))
         {
      ...
      else if(webData.action !== undefined && webData.action === 'deleteuser')
      {
         ...
         {
            var banned = false;
            var res2 = await client.query('SELECT * FROM users WHERE username= $1 AND banned<>\'0\'', [webData.user]); //DATABASE CONNECTION ARE IN DIFFERENT THREADS, NEED MANUAL TERAPY
      ...
      else if(webData.action !== undefined && webData.action === 'updatetip')
      {
         if(webData.content !== undefined && webData.content.length > 9 && webData.content.length <= 5000 && webData.title !== undefined && webData.title.length > 4 && webData.title.length < 200 && webData.post1 !== undefined && clean(webData.post1))
         {
            client.query('UPDATE tips SET title=$1, content=$2 WHERE pid = $3', [webData.title, webData.content, webData.post1]);
            const obj = `{"response": "0"}`;
            res.send(obj);
         }
      }
      ...
      else if(webData.action !== undefined && webData.action === 'addtip')
      {
         if(webData.content !== undefined && webData.content.length > 9 && webData.content.length <= 5000 && webData.title !== undefined && webData.title.length > 4 && webData.title.length < 200)
         {
            client.query('INSERT INTO tips (title, content) values($1, $2)', [webData.title, webData.content]);
            const obj = `{"response": "0"}`;
            res.send(obj);
         }
      }
...
app.post('/page_loader', async (req, res) => {
   ...
   if(webData.info !== undefined && webData.info === "accountpage")
   {
      if(req.session.username)
      {
         ...
         var res4 = await client.query('SELECT COUNT(*) as c FROM likes WHERE pid=ANY(SELECT pid FROM posts WHERE postedby=$1)', [req.session.username]); //DATABASE CONNECTION ARE IN DIFFERENT THREADS, NEED MANUAL TERAPY
         ...
         if(req.session.username)
         {
            ...
            for(var i=0;i<res1.rows.length;i++)
            {
               if(res1.rows[i]["postedby"]!==req.session.username)
               {
                  ...
                  var res5 = await client.query('SELECT * FROM follows WHERE username = $1 AND follows = $2',[req.session.username,res1.rows[i]["postedby"]]); //DATABASE CONNECTION ARE IN DIFFERENT THREADS, NEED MANUAL TERAPY
                  if (res5.rows.length === 0)
                  {
                     var res6 = await client.query('SELECT * FROM shares WHERE pid = $1 AND username = ANY(SELECT follows from follows WHERE username = $2) LIMIT 1',[res1.rows[i]["pid"],req.session.username]); //DATABASE CONNECTION ARE IN DIFFERENT THREADS, NEED MANUAL TERAPY
